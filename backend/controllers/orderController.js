...
const createOrder = asyncHandler(async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      const { productsById, productIds } = orderItems.reduce(
        (acc, product) => {
      ...
      const result = await Promise.all(
        products.map((productDoc) => {
          return (async () => {
            ...
            if (productDoc.countInStock < orderProduct.qty)
              throw new Error(`${productDoc.name} is out of stock!`);
            if (productPrice !== orderProductPrice)
              throw new Error(
                `${productDoc.name} has price mismatch! Current price: ${productDoc.price}, provided price: ${orderProduct.price}`
              );

            productDoc.countInStock =
              productDoc.countInStock - orderProduct.qty;
